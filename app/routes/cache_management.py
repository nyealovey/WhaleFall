...
def clear_user_cache():
    ...
    try:
        ...
        if not instance_id or not username:
            return jsonify({
                'success': False,
                'message': '缺少必要参数: instance_id 和 username'
            }), 400
        ...
        return jsonify({
            'success': success,
            'message': '用户缓存清除成功' if success else '用户缓存清除失败'
        })
        
    except Exception as e:
        return jsonify({
            'success': False,
            'message': f'清除用户缓存失败: {str(e)}'
        }), 500
...
def clear_instance_cache():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': '实例缓存清除成功' if success else '实例缓存清除失败'
        })
        
    except Exception as e:
        return jsonify({
            'success': False,
            'message': f'清除实例缓存失败: {str(e)}'
        }), 500
