...
def get_databases_aggregations():
    ...
    try:
        ...
        target_period = (period_type or '').lower()
        if get_all and (target_period == 'daily' or not data):
            from collections import defaultdict
            from app.models.database_size_stat import DatabaseSizeStat

            aggregator_map: Dict[tuple, Dict[str, Any]] = {}
            for item in data:
                period_start = item.get('period_start')
                instance_info = item.get('instance') or {}
                key = (
                    instance_info.get('id') or item.get('instance_id'),
                    item.get('database_name'),
                    period_start,
                )
                aggregator_map[key] = item

            stat_query = DatabaseSizeStat.query.join(Instance)
            if instance_id:
                stat_query = stat_query.filter(DatabaseSizeStat.instance_id == instance_id)
            if db_type:
                stat_query = stat_query.filter(func.lower(Instance.db_type) == db_type.lower())
            if database_name:
                stat_query = stat_query.filter(DatabaseSizeStat.database_name == database_name)
            if start_date:
                stat_query = stat_query.filter(
                    DatabaseSizeStat.collected_date >= datetime.strptime(start_date, '%Y-%m-%d').date()
                )
            if end_date:
                stat_query = stat_query.filter(
                    DatabaseSizeStat.collected_date <= datetime.strptime(end_date, '%Y-%m-%d').date()
                )
            ...
            stats_grouped: Dict[tuple, List[DatabaseSizeStat]] = defaultdict(list)
            for stat in stats:
                stats_grouped[(stat.instance_id, stat.database_name)].append(stat)

            for (inst_id, db_name), stat_list in stats_grouped.items():
                stat_list.sort(key=lambda s: s.collected_date)
                prev_size = None
                for stat in stat_list:
                    period_start = stat.collected_date.isoformat()
                    key = (inst_id, db_name, period_start)
                    entry = aggregator_map.get(key)
                    instance_obj = getattr(stat, "instance", None)
                    instance_info = {
                        'id': inst_id,
                        'name': instance_obj.name if instance_obj else None,
                        'db_type': instance_obj.db_type if instance_obj else None,
                    }
                    ...
                    if not entry:
                        entry = {
                            ...
                            'avg_size_mb': stat.size_mb,
                            'max_size_mb': stat.size_mb,
                            'min_size_mb': stat.size_mb,
                            'data_count': 1,
                            'avg_data_size_mb': stat.data_size_mb,
                            'max_data_size_mb': stat.data_size_mb,
                            'min_data_size_mb': stat.data_size_mb,
                            'avg_log_size_mb': stat.log_size_mb,
                            'max_log_size_mb': stat.log_size_mb,
                            'min_log_size_mb': stat.log_size_mb,
                            'size_change_mb': 0,
                            'size_change_percent': 0,
                            'data_size_change_mb': None,
                            'data_size_change_percent': None,
                            'log_size_change_mb': None,
                            'log_size_change_percent': None,
                            'growth_rate': 0,
                            'calculated_at': stat.collected_at.isoformat() if stat.collected_at else None,
                            'created_at': stat.created_at.isoformat() if stat.created_at else None,
                            'instance': instance_info,
                        }
                        aggregator_map[key] = entry
                    else:
                        if not entry.get('avg_size_mb'):
                            entry['avg_size_mb'] = stat.size_mb
                            entry['max_size_mb'] = stat.size_mb
                            entry['min_size_mb'] = stat.size_mb
                            entry['data_count'] = entry.get('data_count') or 1
                        if entry.get('avg_size_mb') == 0 and stat.size_mb:
                            entry['avg_size_mb'] = stat.size_mb
                            entry['max_size_mb'] = stat.size_mb
                            entry['min_size_mb'] = stat.size_mb
                        entry.setdefault('avg_data_size_mb', stat.data_size_mb)
                        entry.setdefault('max_data_size_mb', stat.data_size_mb)
                        entry.setdefault('min_data_size_mb', stat.data_size_mb)
                        entry.setdefault('avg_log_size_mb', stat.log_size_mb)
                        entry.setdefault('max_log_size_mb', stat.log_size_mb)
                        entry.setdefault('min_log_size_mb', stat.log_size_mb)
                        entry.setdefault('instance', instance_info)

                    if prev_size is not None and prev_size != 0:
                        change_mb = stat.size_mb - prev_size
                        entry['size_change_mb'] = change_mb
                        change_percent = (change_mb / prev_size) * 100 if prev_size else 0
                        entry['size_change_percent'] = round(change_percent, 2)
                        entry['growth_rate'] = round(change_percent, 2)
                    else:
                        entry.setdefault('size_change_mb', 0)
                        entry.setdefault('size_change_percent', 0)
                        entry.setdefault('growth_rate', 0)

                    prev_size = stat.size_mb

            if stats_grouped:
                data = sorted(
                    aggregator_map.values(),
                    key=lambda item: (
                        item.get('instance', {}).get('id') or item.get('instance_id'),
                        item.get('database_name'),
