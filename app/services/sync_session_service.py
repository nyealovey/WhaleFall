    ...
    def recover_stuck_instances(self, timeout_minutes: int = 10) -> dict[str, Any]:
        ...
        try:
            ...
            stuck_records = (
                ...
                .filter(
                    SyncInstanceRecord.status == "running",
                    SyncInstanceRecord.started_at < timeout_threshold
                )
                .all()
            )
            ...
            for record in stuck_records:
                try:
                    # 标记为失败
                    record.fail_sync(
                        error_message=f"实例同步超时（超过{timeout_minutes}分钟），自动恢复",
                        sync_details={"recovery_reason": "timeout", "timeout_minutes": timeout_minutes}
                    )
            ...
            return {
                "success": True,
                "message": f"成功恢复 {recovered_count} 个卡住的实例",
