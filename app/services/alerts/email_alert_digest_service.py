...
ALERT_RULES = (
    ...
    ("cluster_status_issue", "群集状态异常", "cluster_status_enabled"),
...
class EmailAlertDigestService:
    ...
    def _render_event_line(alert_type: str, payload: dict[str, object]) -> str:
        ...
        if alert_type == "cluster_status_issue":
            return f"{payload.get('cluster_type')} / {payload.get('cluster_name')} - {payload.get('summary_text')}"
