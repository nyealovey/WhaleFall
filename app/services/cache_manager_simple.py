"""
鲸落 - 简化缓存管理器
基于Flask-Caching的简化缓存管理器，用于业务特定缓存
"""
...
class SimpleCacheManager:
    ...
    def clear_user_cache(self, instance_id: int, username: str) -> bool:
        """清除用户缓存"""
        # Flask-Caching不支持模式匹配，简化实现
        # 这里返回True，表示操作成功
    ...
    def clear_instance_cache(self, instance_id: int) -> bool:
        """清除实例缓存"""
        # Flask-Caching不支持模式匹配，简化实现
        # 这里返回True，表示操作成功
...
# 全局缓存管理器实例
simple_cache_manager = None


def init_simple_cache_manager(cache: Cache) -> None:
    """初始化简化缓存管理器"""
    global simple_cache_manager
    simple_cache_manager = SimpleCacheManager(cache)
    logger.info("简化缓存管理器初始化完成")
