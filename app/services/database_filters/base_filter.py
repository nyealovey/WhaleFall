...
class BaseDatabaseFilter(ABC):
    ...
    def should_include_user(self, username: str) -> bool:
        ...
        # 应用自定义规则
        return all(self._apply_custom_rule(username, rule) for rule in self.custom_rules)
    ...
    def _apply_custom_rule(self, _username: str, _rule: dict[str, Any]) -> bool:
        """
        应用自定义规则
    ...
    def build_where_clause(self, username_field: str) -> tuple[str, list[Any]]:
        ...
        where_clause = " AND ".join(conditions) if conditions else "1=1"  # 如果没有条件，返回恒真条件

        return where_clause, params

    def _build_custom_condition(self, _username_field: str, _rule: dict[str, Any]) -> tuple[str, list[Any]]:
        """
        构建自定义规则的WHERE条件
