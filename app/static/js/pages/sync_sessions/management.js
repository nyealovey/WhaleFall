    ...
    fetch(`/sync_sessions/api/sessions?${params.toString()}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                showAlert('error', '加载会话列表失败: ' + data.message);
            }
        })
        .catch(error => {
            console.error('加载会话列表出错:', error);
            showAlert('error', '加载会话列表出错');
        });
}
...
function renderSessions(sessions) {
    ...
    const html = sessions.map(session => {
        ...
        // 根据成功率确定进度条颜色和样式
        const progressInfo = getProgressInfo(session.status, progressPercentage, totalInstances, successfulInstances, failedInstances);
    ...
    fetch(`/sync_sessions/api/sessions/${sessionId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                showAlert('error', '加载会话详情失败: ' + data.message);
            }
        })
        .catch(error => {
            console.error('加载会话详情出错:', error);
            showAlert('error', '加载会话详情出错');
        });
}
    ...
    fetch(`/sync_sessions/api/sessions/${sessionId}/error-logs`)
        ...
        .then(data => {
            if (data.success) {
                ...
                showAlert('error', '加载错误信息失败: ' + data.message);
            }
        })
        .catch(error => {
            console.error('加载错误信息出错:', error);
            showAlert('error', '加载错误信息出错');
        });
}
    ...
    if (confirm('确定要取消这个同步会话吗？')) {
        ...
        .then(data => {
            if (data.success) {
                ...
                showAlert('error', '取消会话失败: ' + data.message);
            }
        })
        .catch(error => {
            console.error('取消会话出错:', error);
            showAlert('error', '取消会话出错');
        });
    }
}
...
// 获取进度条信息
function getProgressInfo(status, progressPercentage, totalInstances, successfulInstances, failedInstances) {
    if (totalInstances === 0) {
        return {
