...
def parse_account_scope(raw_value: object) -> AccountScope | None:
    """解析前端传入的账户范围.

    支持 `instance:<id>` / `sqlserver_ag:<id>`。
    """
    if raw_value is None:
        return None
    if not isinstance(raw_value, str):
        raise ValidationError("account_scope 必须为字符串")

    normalized_value = raw_value.strip()
    if not normalized_value:
        return None

    owner_type, sep, owner_id_raw = normalized_value.partition(":")
    if sep != ":":
