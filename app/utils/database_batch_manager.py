...
class DatabaseBatchManager:
    ...
    def __init__(self, batch_size: int = 100, logger: Optional[Any] = None, instance_name: str = ""):
        """
        初始化批量管理器
        ...
        try:
            ...
            for operation in self.pending_operations:
                try:
                    ...
                    self.logger.error(
                        f"批次操作失败: {operation['description']}",
                        module="database_batch_manager",
                        instance_name=self.instance_name,
                        error=str(op_error)
                    )
