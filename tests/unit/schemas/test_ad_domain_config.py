...
def _payload(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
        "name": "user.whalefall.local",
        "netbios_name": "USER",
        "domain_controllers": ["192.168.0.236"],
        "ldap_port": 389,
        "use_ssl": False,
        "verify_ssl": False,
        "base_dn": "DC=user,DC=whalefall,DC=local",
        "credential_id": 1,
        "is_enabled": True,
    }
...
def test_ad_domain_config_allows_ou_base_dn_with_split_dc_labels() -> None:
    payload = validate_or_raise(
        ...
        _payload(base_dn=" OU=Users,DC=user,DC=whalefall,DC=local "),
    )

    assert payload.base_dn == "OU=Users,DC=user,DC=whalefall,DC=local"
...
def test_ad_domain_config_rejects_dc_component_with_dot() -> None:
    with pytest.raises(ValidationError) as excinfo:
        validate_or_raise(
            AdDomainConfigPayload,
            _payload(base_dn="DC=user,DC=whalefall.local,DC=com"),
        )
