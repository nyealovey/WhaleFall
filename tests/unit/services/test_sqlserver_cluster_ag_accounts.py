    ...
    db.session.add(
        AccountPermission(
            ...
            permission_facts={
                "version": 2,
                "db_type": "sqlserver",
                "capabilities": ["SUPERUSER"] if is_superuser else [],
                "capability_reasons": {"SUPERUSER": ["test"]} if is_superuser else {},
...
def test_instance_ag_accounts_lists_contained_ag_accounts_for_same_cluster(app) -> None:
    with app.app_context():
        ...
        _add_account(instance=instance, ag=included_ag, username="same_name", is_superuser=True)
