...
def test_scheduler_builtin_task_ids_and_functions_use_short_names() -> None:
    ...
    assert "sync_ad_accounts" in task_ids
    ...
    assert "sync_ad_accounts" in function_names
    ...
    assert {"email_alert", "calculate_database", "calculate_account", "sync_ad_accounts", "sync_veeam_backups"} <= BUILTIN_TASK_IDS
    ...
    assert "sync_ad_accounts" in scheduler.TASK_FUNCTIONS
