...
@pytest.mark.unit
def test_common_instances_options_contract(app, auth_client) -> None:
    ...
    response = auth_client.get("/common/api/instances-options")
    ...
    filtered = auth_client.get("/common/api/instances-options?db_type=MYSQL")
    assert filtered.status_code == 200


@pytest.mark.unit
def test_common_databases_options_contract(app, auth_client) -> None:
    with app.app_context():
        db.metadata.create_all(
            bind=db.engine,
            tables=[
    ...
    response = auth_client.get(f"/common/api/databases-options?instance_id={instance_id}&limit=100&offset=0")
    assert response.status_code == 200
...
@pytest.mark.unit
def test_common_dbtypes_options_contract(app, auth_client) -> None:
    with app.app_context():
        db.metadata.create_all(
            bind=db.engine,
            tables=[
    ...
    response = auth_client.get("/common/api/dbtypes-options")
    assert response.status_code == 200
