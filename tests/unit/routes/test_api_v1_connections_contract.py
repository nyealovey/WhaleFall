...
def test_api_v1_connections_requires_auth(client) -> None:
    response = client.get("/api/v1/connections/status/1")
...
def test_api_v1_connections_validate_params_contract(app, auth_client) -> None:
    ...
    response = auth_client.post(
        "/api/v1/connections/actions/validate-params",
        json={
            "db_type": "mysql",
            "port": 3306,
...
@pytest.mark.unit
def test_api_v1_connections_batch_test_contract(app, auth_client) -> None:
    csrf_token = _get_csrf_token(auth_client)
    response = auth_client.post(
        "/api/v1/connections/actions/batch-test",
...
def test_api_v1_connections_status_contract(app, auth_client) -> None:
    ...
    response = auth_client.get(f"/api/v1/connections/status/{instance_id}")
    assert response.status_code == 200
