...
def test_api_v1_instances_list_contract() -> None:
    ...
    with app.app_context():
        ...
        assert item.get("backup_status") == "not_backed_up"
        assert item.get("backup_last_time") is None


@pytest.mark.unit
def test_api_v1_instances_list_marks_backup_status_via_instance_name_and_domains() -> None:
    app = create_app(init_scheduler_on_start=False)
    app.config["TESTING"] = True
