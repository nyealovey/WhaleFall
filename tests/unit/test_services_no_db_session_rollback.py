...
def _collect_db_session_aliases(body: list[ast.stmt], *, db_names: set[str]) -> set[str]:
    """收集 `session = db.session` 这类别名，降低误报/漏报（不做复杂数据流分析）。"""
...
@pytest.mark.unit
def test_services_do_not_call_db_session_rollback() -> None:
    """Write boundary: services 层不得回滚整个 session(应由边界入口处理)."""
