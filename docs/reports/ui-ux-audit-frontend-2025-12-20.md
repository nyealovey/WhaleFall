...
1) 登录与账户安全（高频/入口）
- 打开登录页 → 输入用户名/密码 → 登录（成功进入仪表盘 / 失败提示）
- 修改密码 → 退出登录

2) 实例管理（后台核心/高价值）
- 进入“实例管理”列表 → 使用 FilterCard 组合筛选（含标签筛选）→ 查看列表结果
- 打开实例详情 → 测试连接 → 同步账户/容量
- 批量导入/批量删除/批量测试连接 → 导出 CSV

3) 凭据管理（配置型/影响全局）
- 进入“凭据管理”列表 → 筛选/定位凭据
- 新增/编辑凭据 → 保存 → 启用/停用（如支持）→ 删除（确认）

4) 标签治理（可发现性 + 运营效率）
- 进入“标签管理”列表 → 新增/编辑/停用标签
- 进入“批量分配标签” → 批量绑定实例 → 回到实例列表按标签筛选验证效果

5) 任务结果闭环（同步/导出/排障）
- 在“实例/账户/定时任务”等入口触发后台任务（同步/统计/清理等）
- 进入“会话中心”筛选查看进度/结果 → 查看会话详情（失败原因/错误日志）→ 取消/重试（如支持）
- 进入“日志中心”按模块/级别/时间范围定位 → 查看日志详情/复制堆栈
...
C. 进行“体验一致性扫描”（证据取样）
- 按钮层级：outline 体系被全局覆盖，弱化主次/危险识别（`app/static/css/global.css:143`）。
...
- 证据：`app/static/css/global.css:143`（`.btn { ... border: none; }` 会覆盖 Bootstrap 的 outline 边框体系）；典型使用：`app/templates/instances/detail.html:145`（多处 `btn-outline-secondary` 作为次按钮/快速操作）
...
- 证据：实例列表“批量删除”直接触发：`app/templates/instances/list.html:59` + `app/static/js/modules/views/instances/list.js:903`（无 confirm/范围说明）；账户台账“同步所有账户”直接触发：`app/templates/accounts/ledgers.html:20` + `app/static/js/modules/views/accounts/ledgers.js:1022`
