...
- `sync_sessions` / `sync_instance_records`
  ...
  - 已修复: 引入 `SyncSessionStatus` 作为代码侧单一事实源,并通过增量迁移统一默认值/约束(见 III-P0-4)
...
- 证据:
  - 代码单一事实源: `SyncSessionStatus`: `app/constants/status_types.py:90`
  - ORM 默认: `default=SyncSessionStatus.RUNNING`: `app/models/sync_session.py:50`
  - ORM 校验: `@validates("status")`: `app/models/sync_session.py:72`
  - DB 默认/Check 统一迁移: `migrations/versions/20251224152000_unify_sync_sessions_status_default_and_check.py:22`
- 影响:
...
- 建议:
  ...
  - 中期重构(1~2 周):
    - 移除 INSERT 触发器中的 DDL,改为“运维命令”管理分区; 统一只在父表维护触发器.
