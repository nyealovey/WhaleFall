...
| 文件路径 | 行数 | 功能说明 |
|---------|------|---------|
| routes/instance.py | 569 | 实例管理路由 |
| routes/instance_detail.py | 568 | 实例详情路由 |
...
| routes/instance_aggr.py | 322 | 实例聚合路由 |
...
| routes/database_aggr.py | 201 | 数据库聚合路由 |
...
| services/account_sync/adapters/sqlserver_adapter.py | 586 | SQL Server适配器 |
...
| services/account_sync/adapters/postgresql_adapter.py | 346 | PostgreSQL适配器 |
| services/statistics/account_statistics_service.py | 335 | 账户统计服务 |
| services/account_sync/account_sync_service.py | 330 | 账户同步服务 |
| services/account_sync/adapters/mysql_adapter.py | 326 | MySQL适配器 |
| services/account_sync/coordinator.py | 290 | 同步协调器 |
...
| services/form_service/scheduler_job_form_service.py | 200 | 调度任务表单服务 |
| services/account_sync/adapters/oracle_adapter.py | 192 | Oracle适配器 |
...
| services/form_service/credentials_form_service.py | 160 | 凭据表单服务 |
| services/account_classification/repositories.py | 157 | 分类仓储层 |
| services/form_service/instances_form_service.py | 156 | 实例表单服务 |
...
| tasks/account_sync_tasks.py | ~300 | 账户同步任务 |
...
**实例管理**（1,137行）：
- `instance.py`: 569行 - 实例CRUD、批量操作
- `instance_detail.py`: 568行 - 实例详情、连接测试、权限查看
...
- `instance_aggr.py`: 322行- 实例聚合查询
- `database_aggr.py`: 201行 - 数据库聚合查询
...
**账户同步服务** (account_sync/) - 约3,200行：
- 适配器层：SQL Server(586行)、PostgreSQL(346行)、MySQL(326行)、Oracle(192行)
- 核心服务：`permission_manager.py`(571行)、`account_sync_service.py`(330行)
- 协调层：`coordinator.py`(290行)、`inventory_manager.py`
- 查询服务：`account_query_service.py`、`account_sync_filters.py`
...
- `scheduler_job_form_service.py`: 200行 - 调度任务表单
- `credentials_form_service.py`: 160行 - 凭据表单
- `instances_form_service.py`: 156行 - 实例表单
- `classification_form_service.py`、`tags_form_service.py`、`users_form_service.py`
...
- `account_classification_form_view.py` - 账户分类表单
- `scheduler_job_form_view.py` - 调度任务表单
- `tag_form_view.py` - 标签表单
- `user_form_view.py` - 用户表单
- `change_password_form_view.py` - 修改密码表单
...
| modules/views/tags/batch_assign.js | 866 | 批量标签分配 |
