...
- 台账导出: `GET /api/v1/files/database-ledger-export`(数据源与本域一致, route 在 files namespace).
...
flowchart LR
  subgraph API["API (Flask-RESTX)"]
    ...
    Export["GET /api/v1/files/database-ledger-export"]
  end
...
| --- | --- | --- | --- |
| GET | `/api/v1/databases/ledgers` | `database_ledger.view` | query: `search`, `db_type`(default all), `tags`(repeatable), `page`, `limit`. |
...
| Method | Path | Permission | Notes |
| --- | --- | --- | --- |
| GET | `/api/v1/files/database-ledger-export` | `database_ledger.view` | export CSV, query: `search`, `db_type`, `tags`. |
