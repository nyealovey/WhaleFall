...
- 日志落库模型: `app/models/unified_log.py` (API 读取见 `/api/v1/history/logs/*`).
...
| GET | `/api/v1/health/health` | no | DB/cache 探测 + uptime. |
...
## 5. API 契约: `/api/v1/history/logs`

权限: endpoints 目前使用 `api_permission_required("admin")`, 以 `has_permission` 的现状实现来看等价于 admin-only.

| Method | Path | Notes |
| --- | --- | --- |
| GET | `/api/v1/history/logs/list` | 日志列表(支持 filters). |
| GET | `/api/v1/history/logs/search` | 日志搜索(同 list, message 不同). |
| GET | `/api/v1/history/logs/statistics` | 日志统计, 参数: `hours` (1..2160, 默认 24). |
| GET | `/api/v1/history/logs/modules` | module 列表. |
| GET | `/api/v1/history/logs/detail/{log_id}` | 单条日志详情. |

### 5.1 查询参数(适用于 list/search)
...
| POST | `/api/v1/cache/clear/user` | `admin` | yes | payload: `instance_id`, `username`. |
| POST | `/api/v1/cache/clear/instance` | `admin` | yes | payload: `instance_id`. |
| POST | `/api/v1/cache/clear/all` | `admin` | yes | best-effort 清理所有 active instances. |
| POST | `/api/v1/cache/classification/clear` | `update` | yes | 清理分类规则缓存. |
| POST | `/api/v1/cache/classification/clear/{db_type}` | `update` | yes | db_type in `mysql|postgresql|sqlserver|oracle`. |
| GET | `/api/v1/cache/classification/stats` | `view` | no | 分类缓存命中情况. |

## 7. 排障最短路径

1. 先从客户端拿到 `request_id`/`error_id`(来自统一错误封套).
2. 用 `GET /api/v1/history/logs/search?search=<request_id>&hours=24` 反查日志上下文.
