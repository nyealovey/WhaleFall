...
#### 问题1：缺少实例-账户关系表
- **现状**：账户数据直接存储在 `account_permission` 表中，通过 `instance_id` 和 `username` 关联
...
- **账户同步**：`instances` → `account_permission`（缺少中间关系表）
...
当前实现：
```python
# 在 account_permission 中标记 is_deleted=True
...
当前实现：
- 需要按 username 分组查询 `account_permission`
- 无法快速定位账户的分布情况
...
class CurrentAccountSyncData(BaseSyncData):
    ...
    # 关联实例账户
    instance = db.relationship("Instance", backref="account_permission")
```
...
3. 更新 account_permission 表（存储权限数据）
   - 比对权限变化
...
def upgrade():
    ...
    # 从 account_permission 迁移数据到 instance_accounts
...
```python
def upgrade():
    # 移除 account_permission 表中的删除标记字段
    # 注意：建议先保留这些字段，确认新架构稳定后再删除
    # op.drop_column('account_permission', 'is_deleted')
    # op.drop_column('account_permission', 'deleted_time')
    pass
```
...
#### 阶段2：并行运行（双写）
1. 同步时同时更新 `account_permission.is_deleted` 和 `instance_accounts.is_active`
2. 监控数据一致性
3. 修复发现的问题

#### 阶段3：切换到新架构
1. 修改所有查询账户状态的代码，从 `instance_accounts` 读取
2. 停止更新 `account_permission.is_deleted`
3. 验证功能正常

#### 阶段4：清理（可选）
1. 移除 `account_permission` 中的 `is_deleted` 和 `deleted_time` 字段
2. 清理相关的索引
3. 更新文档
...
### 4.4 维护性提升
- 职责分离，`instance_accounts` 管理存在状态，`account_permission` 管理权限数据
- 代码逻辑更清晰
- 便于扩展新功能
...
2. 恢复使用 `account_permission.is_deleted`
3. 分析问题原因
4. 修复后重新部署
