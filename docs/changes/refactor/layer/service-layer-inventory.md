...
| --- | --- | --- | --- | --- |
| S0-1 | **SyncSessionsService** | `GET /sync_sessions/api/sessions`, `GET /sync_sessions/api/sessions/:id`, `GET /sync_sessions/api/sessions/:id/error-logs`, `POST /sync_sessions/api/sessions/:id/cancel` | `modules/views/history/sessions/sync-sessions.js` | ✅ 服务层已实现（`modules/services/sync_sessions_service.js`）并在会话页面接入，后续可继续拆分 store/视图。 |
| S0-2 | **TagManagementService** | `GET /tags/api/tags`, `GET /tags/api/categories`, `GET /tags/api/instances`, `GET /tags/api/all_tags`, `POST /tags/api/batch_assign_tags`, `POST /tags/api/batch_remove_all_tags`, `POST /tags/api/batch_delete`, `POST /tags/api/delete/:id` | `components/tag_selector.js`, `pages/tags/batch-assign.js`, `pages/tags/index.js`, `pages/accounts/list.js`(通过 TagSelector) | ✅ 已抽离为 `modules/services/tag_management_service.js`，并在标签组件/页面接入。 |
...
| S0-4 | **SchedulerService** | `/scheduler/api/jobs`, `/scheduler/api/jobs/:id/(resume|pause|run)`, `/scheduler/api/jobs/reload` | `pages/admin/scheduler.js` | ✅ 已由 `modules/services/scheduler_service.js` 承担，页面抛弃 `$.ajax`。 |
| S0-5 | **InstanceManagementService** | `/accounts_sync/api/instances/:id/sync`, `/databases/api/instances/:id/sync-capacity`, `/instances/api/...`（账户历史、数据库容量、批量操作、统计） | `pages/instances/detail.js`, `pages/instances/list.js`, `pages/instances/statistics.js`, `pages/accounts/list.js` | ✅ `modules/services/instance_management_service.js` 接管实例/容量/批量/API，账户列表同步 `syncAllAccounts` 复用该服务。 |
| S0-6 | **AccountSyncService** | `POST /accounts_sync/api/sync-all` | `pages/accounts/list.js` | ✅ 已并入 InstanceManagementService（`syncAllAccounts()`）。 |
...
| S0-9 | **LogsService** | `GET /history/logs/api/modules`, `/history/logs/api/statistics`, `/history/logs/api/search`, `/history/logs/api/list`, `/history/logs/api/detail/:id` | `pages/history/logs.js` | ✅ `modules/services/logs_service.js`。 |
...
| S0-11 | **CapacityStatsService** | 配置化调用 `/instance_aggregations/api/...`, `/database_aggregations/api/...`, `/capacity/api/aggregations/...` | `pages/capacity_stats/*.js`, `common/capacity_stats/data_source.js` | ✅ `modules/services/capacity_stats_service.js` ＋ data_source 重写。 |
...
4. **同步文档**：迁移完成的领域在本清单中标记状态（例如 ✅ 完成 / 🔄 进行中），并补充到 `docs/changes/refactor/layer/frontend-script-refactor-plan.md` 的 S0 进度中。
