...
| V1-2 | **TagManagementViews** | 标签列表（`tags/index.js`）、标签表单（`tags/form.js`）、批量分配（`tags/batch-assign.js`）及 TagSelector 组件均在 `modules/views/`；账户列表页面也复用该视图。 | `TagManagementStore`, `TagManagementService` | ✅ 完成；仍依赖全局 FormValidator，未来需改为局部校验。 |
...
- 文档（本文件 + `docs/changes/refactor/layer/frontend-script-refactor-plan.md`）更新迁移状态，PR 描述同步列出涉及的 view 组件。
...
- **UI 组件化**：FilterCard 已替换所有筛选列表，`UI.createModal` + `components/ui/modal.html` 已在凭据删除、标签删除、日志详情、实例批量创建与同步会话详情/错误日志场景落地，下一步继续扩展实例详情等模态；进度记录同步到 `docs/changes/refactor/layer/ui-componentization-plan.md`，方便每次推广都有文档依据。
