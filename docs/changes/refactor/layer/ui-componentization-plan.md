...
2. **Modal 组件（逐步推广中）**
  ...
  - 已接入：凭据删除（credentials/list）、标签删除（tags/index）、日志详情查看（history/logs）、实例批量创建（instances/list）、同步会话详情与错误日志（history/sessions）、账户分类（创建/编辑分类、创建/编辑/查看规则）、分区管理（创建/清理）。
...
每个任务需同步更新 `docs/changes/refactor/layer/view-layer-inventory.md` 与示例文档（截图/说明）。
...
- 结合 `docs/changes/refactor/codebase-line-reduction-plan.md` 的阶段计划，记录完成度与代码减重数据。
