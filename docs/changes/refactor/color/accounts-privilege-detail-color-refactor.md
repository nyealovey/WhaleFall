...
## 背景
- `账户权限详情` 弹窗沿用早期纯色 UI：顶栏、角色标签使用高饱和 HEX（`#ff5a00`、`#ff6b00` 等）+ 100% 实底，完全脱离《界面色彩与视觉疲劳控制指南》（`docs/standards/ui/color-guidelines.md`）提出的“色彩 2-3-4 规则”。
...
- 在 `docs/standards/ui/color-guidelines.md` 的“组件级指导-状态与徽章”下追加案例截图，提醒所有权限/详情模态必须使用 chip/pill 体系。
