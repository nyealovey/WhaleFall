...
> 注：`error/message` 字段漂移已按 `docs/standards/backend/error-message-schema-unification.md` 统一并清零命中；
...
- 位置：`app/routes/accounts/sync.py:114`
  ...
  - 建议：统一以 `docs/standards/backend/error-message-schema-unification.md` 作为结果结构契约，避免在读端写互兜底链。
...
- 位置：`app/services/account_classification/auto_classify_service.py:133`
  ...
  - 建议：将 `docs/standards/backend/error-message-schema-unification.md` 作为分类引擎返回结构契约，避免在中间层扩散兼容分支。
