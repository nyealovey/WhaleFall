...
related:
  ...
  - "[[reference/server/accounts-sync-overview|Accounts Sync 总览]]"
  - "[[reference/server/accounts-sync-adapters|Accounts Sync Adapters]]"
  - "[[reference/server/accounts-permissions-facts-builder|Accounts Permissions Facts Builder]]"
...
- 持久化:
  ...
  - `AccountPermission.permission_facts` (由 `build_permission_facts()` 生成, 细节见 [[reference/server/accounts-permissions-facts-builder|Facts Builder]])
  - `AccountChangeLog.privilege_diff` / `AccountChangeLog.other_diff`

> [!tip] 上下文入口
> - Accounts Sync 域整体链路: [[reference/server/accounts-sync-overview|Accounts Sync 总览]]
> - Adapter 差异与 SQL 分支: [[reference/server/accounts-sync-adapters|Accounts Sync Adapters]]
