---
title: services/utils 索引
aliases:
  - services-utils-documentation
tags:
  - reference
  - reference/api
status: active
...
scope: "`app/services/**`, `app/utils/**` 的符号索引与引用概览(静态搜索)"
related:
...
> [!info] Purpose
> 本文按文件列出 `app/services/**` 与 `app/utils/**` 中的主要符号, 便于从路由/任务反查实现. 仅用于查阅, 不作为稳定接口契约.

> [!warning] Limitations
> "引用情况" 来自静态搜索, 可能存在漏报/误报(动态导入, 同名符号, 反射调用等).

> [!info] Related standards (SSOT)
> - [[standards/backend/layer/api-layer-standards#响应封套(JSON Envelope)|API response envelope]]
> - [[standards/backend/error-message-schema-unification|error/message unification]]
...
- 重构/迁移时如需短期保留旧入口, 建议只在边界层做一次适配/规范化, 避免在多层扩散兼容分支(参见 [[standards/backend/error-message-schema-unification]]).
