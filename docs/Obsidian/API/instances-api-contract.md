...
| GET    | `/api/v1/instances/imports/template`                    | 下载导入模板（CSV）        | -                                                           | `view`                                            | -    | TODO: move to `InstancesImportTemplateService.build_template_csv`        |
...
| POST   | `/api/v1/instances/{instance_id}/actions/sync-capacity` | 同步实例容量             | `CapacitySyncCoordinator`<br>`AggregationService`           | `update`                                          | ✅    | 可能返回 409：`DATABASE_CONNECTION_ERROR`/`SYNC_DATA_ERROR`                   |
