...
> [!info]
> JSON 封套口径见: [[standards/backend/gate/layer/api-layer#响应封套(JSON Envelope)|API 响应封套]].
...
| ------ | -------------------------------------------------------------- | ------------------ | ------- | ------------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------- |
| GET    | `/api/v1/databases/options`                                    | 获取实例下数据库选项         | `FilterOptionsService.get_common_databases_options` | `view`                                            | -    | query：`instance_id`（必填）；`page/limit`                                                                  |
| GET    | `/api/v1/databases/ledgers`                                    | 数据库台账列表            | `DatabaseLedgerService.get_ledger` | `view`                                            | -    | query：`search/db_type/instance_id/tags/page/limit`；响应字段为 `data.limit`                                 |
| GET    | `/api/v1/databases/ledgers/exports`                            | 导出数据库台账（CSV）       | `DatabaseLedgerExportService.export_database_ledger_csv` | `view`                                            | -    | 成功返回 CSV（非 JSON）；失败仍为 JSON Envelope                                                                   |
| GET    | `/api/v1/databases/sizes`                                      | 实例数据库大小（历史/最新）     | `InstanceDatabaseSizesService.fetch_sizes` | `view`                                            | -    | query：`instance_id`（必填）；`start_date/end_date/database_name/latest_only/include_inactive/page/limit`   |
| GET    | `/api/v1/databases/{database_id}/tables/sizes`                 | 表容量快照              | `InstanceDatabaseTableSizesService.fetch_snapshot` | `view`                                            | -    | query：`schema_name/table_name/page/limit`；`limit` 最大 2000                                              |
| POST   | `/api/v1/databases/{database_id}/tables/sizes/actions/refresh` | 刷新表容量快照（采集 + 返回快照） | `TableSizeCoordinator.refresh_snapshot`<br>`InstanceDatabaseTableSizesService.fetch_snapshot` | `update`                                          | ✅    | query 同上；可能返回 409：`DATABASE_CONNECTION_ERROR`/`SYNC_DATA_ERROR`                                       |
