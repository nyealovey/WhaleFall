...
- MUST: Services 只负责:
  ...
  - query/payload 序列化与规整(trim, 类型转换, 默认值补齐).
  - 响应结构规整(从 `resp.data` 取值等)与最小错误转换.
- MUST NOT: 直接操作 DOM(如 `document.querySelector`, `DOMHelpers`).
- MUST NOT: 直接触发 UI 提示(如 `toast.*`)或发全局业务事件(如 `EventBus.emit`).
- SHOULD: 保持 service 方法为"薄"的请求封装, 将编排逻辑放到 store/actions.

### 2) 依赖注入(httpClient)

- MUST: service 构造函数接收 `httpClient`, 并允许回退 `window.httpU`(迁移期).
- MUST: 在构造时校验 client 能力(至少 `get` 存在).
- SHOULD: service 只依赖 `httpClient` 提供的最小接口, 避免绑定具体实现细节.

### 3) API path 与命名

- MUST: API path 以常量集中管理(例如 `BASE_PATH`), 且与后端 contracts 保持一致.
...
- MUST: 文件名使用 `snake_case.js`, 导出 CapWords 类或工厂函数, 并挂载到 `window`.

### 4) query/payload 序列化与安全键过滤

- MUST: 将对象转换为 query string 或写入 payload 前, 必须过滤危险键:
...
- MUST: `window.httpU`(作为默认 httpClient)或由构造器注入的 `httpClient`.
- MAY: `URLSearchParams`, `FormData` 等原生能力.
- MAY: `window.LodashUtils`(仅在必要时, 且避免在 service 内做复杂业务逻辑).

禁止依赖:

- MUST NOT: `window.DOMHelpers`, `document`, `window.toast`, `window.EventBus`
- MUST NOT: `app/static/js/modules/stores/**`, `app/static/js/modules/views/**`
