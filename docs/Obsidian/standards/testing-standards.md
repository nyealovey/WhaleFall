...
- 判定点:
  - fixtures 分层遵循 `tests/conftest.py`(全局) -> `tests/unit/**/conftest.py`(局部)的约定.
  - fixture 的资源申请/释放必须可控(rollback/drop/teardown).
- 长示例见: [[reference/examples/testing-examples#常用 Fixtures 示例|常用 Fixtures 示例(长示例)]]
...
- 判定点:
  - 契约测试只验证结构与字段集合, 不验证业务数据.
  - 对响应 envelope 与 `data` 关键 key 做最小断言, 保持可维护.
- 长示例见: [[reference/examples/testing-examples#API 契约测试示例|API 契约测试示例(长示例)]]
...
| 2026-01-08 | 迁移至 Obsidian vault, 将元信息改为 YAML frontmatter | WhaleFall Team |
