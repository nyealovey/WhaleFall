...
- MUST: 任何降级/failover/workaround 都必须显式记录结构化日志，至少包含：
  ...
  - 关键维度（例如 `action`/`task`/`instance_id` 等，见 [[standards/backend/guide/structured-logging-context-fields]]）
...
- MUST: workaround/fallback 必须满足 [[standards/backend/design/compatibility-and-deprecation]] 的退出机制要求（可判定的删除条件）。
