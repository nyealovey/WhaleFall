...
- **核心 shared kernel（首选落点）**：`app/core/**`
  - 例如：`app/core/exceptions.py`
- **shared-kernel primitives（仍保留为“层”标准）**
  - `app/constants/**`：不可变常量/枚举/静态映射（详见 [[standards/backend/layer/constants-layer-standards]]）
  - `app/types/**`：跨层类型契约/协议/结构（详见 [[standards/backend/layer/types-layer-standards]]）
...
> [!note]
> `app/errors.py` 仅作为历史 import 路径的兼容 re-export；异常的“源位置”为 `app/core/exceptions.py`。
...
- MUST: `app/core/**` 允许依赖 `app/constants/**`、`app/types/**`。
- MUST NOT: `app/constants/**`、`app/types/**` 反向依赖 `app/core/**`（避免 shared kernel 内部环）。
...
- 值/枚举/静态映射 → `app/constants/**`
- 结构/协议/TypedDict/类型别名 → `app/types/**`
