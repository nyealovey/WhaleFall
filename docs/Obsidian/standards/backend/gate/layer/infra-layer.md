...
- MUST: 写路径的 `commit/rollback` 只允许发生在“事务边界入口”，详见 [[standards/backend/standard/write-operation-boundary|写操作事务边界]]。
...
- MUST: 单实例策略与启停策略遵循 [[standards/backend/standard/task-and-scheduler|任务与调度(APScheduler)]]（`ENABLE_SCHEDULER`、reloader 子进程策略、生产 Web/Scheduler 分进程建议）。
