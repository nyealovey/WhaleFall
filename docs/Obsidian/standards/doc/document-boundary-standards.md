...
| `docs/Obsidian/Server/**`       | 服务层文档              | "某个 service 具体怎么工作/失败语义是什么" | 流程图, 决策表, 兼容/兜底清单, 调用方与依赖                   | 对全仓通用的规则 SSOT(应放 standards)                                |
| `docs/Obsidian/API/**`          | API contract(SSOT) | "对外 API 的 endpoint 清单是什么"   | method/path 清单, 关键约束(摘要), 指向实现与 OpenAPI     | 把 contract 写成大段实现说明(应放 Server/architecture)                |
