...
> [!note] 前置条件
> 已完成 [[getting-started/local-dev|本地开发环境搭建]].
...
# 运行单个测试文件
uv run pytest tests/unit/services/test_database_ledger_service.py

# 运行单个测试函数
uv run pytest tests/unit/services/test_database_ledger_service.py::test_format_size_handles_megabytes
```
...
```bash
# 显示完整错误信息
uv run pytest -m unit --tb=long

# 只运行失败的测试
uv run pytest -m unit --lf
...
uv run pytest -m unit -v           # 详细输出
uv run pytest -m unit --lf         # 只运行上次失败的

# 调试
uv run pytest -m unit -x           # 遇到失败就停止
uv run pytest -m unit -s           # 显示 print 输出
uv run pytest -m unit --tb=long    # 完整错误信息
