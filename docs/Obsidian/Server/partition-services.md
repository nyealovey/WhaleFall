...
- `PartitionManagementService.create_future_partitions(months_ahead=3)`：批量创建未来 N 个月（单月失败继续）。`app/services/partition_management_service.py:250`
- `PartitionManagementService.cleanup_old_partitions(retention_months=12)`：清理早于保留期的分区（逐分区尝试，最后汇总失败）。`app/services/partition_management_service.py:316`
...
| 类型 | 组件 | 用途 | 失败语义(摘要) |
| --- | --- | --- | --- |
| DB | `db.session.execute(text(sql))` | 执行 CREATE/DROP/COMMENT/INDEX SQL | SQLAlchemyError 记录并汇总为 `DatabaseError` |
...
- **批量创建未来分区**：
  - 单月失败不阻断其他月份，最后如果 `issues` 非空抛 `DatabaseError`（部分成功/部分失败）。`app/services/partition_management_service.py:281`、`app/services/partition_management_service.py:305`
- **清理旧分区**：
  - 逐分区执行 `DROP TABLE IF EXISTS ...`，失败会记录并 `continue`，最终汇总失败抛 `DatabaseError`。`app/services/partition_management_service.py:355`、`app/services/partition_management_service.py:409`
...
flowchart TB
    ...
    F --> G["CREATE INDEX IF NOT EXISTS ..."]
    G --> H["COMMENT ON TABLE ..."]
    C --> I{failures?}
...
sequenceDiagram
    ...
    loop each table
        ...
        Svc->>DB: execute(CREATE INDEX IF NOT EXISTS ...)
...
实现位置：`app/services/partition_management_service.py:429`。
...
实现位置：`app/services/partition_management_service.py:347`。
...
| `app/services/partition_management_service.py:281` | 回退 | 未来分区：单月失败继续，最后汇总 issues | 部分月份创建失败 | 若要求原子性，改为任一失败即中断并回滚 |
| `app/services/partition_management_service.py:355` | 防御 | `DROP TABLE IF EXISTS` 避免不存在时报错 | 分区已被删除 | 若要严格校验可改为先查存在再 drop |
| `app/services/partition_management_service.py:375` | 防御 | 清理旧分区：异常后继续处理其他分区 | 单个分区 drop 失败 | 若要求强一致，改为失败即中断 |
| `app/services/partition_management_service.py:507` | 防御 | 单条分区信息处理失败不影响总体（continue） | 某条分区元数据异常 | 若需要可观测性，记录失败样本并报警 |
| `app/services/partition_management_service.py:613` | 防御 | 解析分区名失败返回 None | 分区名不符合约定 | 若分区名严格受控，可移除容错并改为强约束 |
| `app/services/partition_management_service.py:631` | 防御 | `_ensure_partition_identifier` 限制表名字符集防 SQL 注入 | 分区名包含非法字符 | 若所有表名都来自内部生成，可保持但补单测 |
| `app/services/partition_management_service.py:651` | 防御 | `int(result or 0)` + 捕获异常返回 0 | 计数查询失败 | 若必须精确统计，改为抛错并提示重试 |
| `app/services/partition_management_service.py:680` | 防御 | date_str 缺失/解析失败 -> status=`unknown` | 分区名异常 | 若命名严格受控可收敛到强约束 |
...
- 清理：`成功删除旧分区` / `删除旧分区失败` `app/services/partition_management_service.py:367`、`app/services/partition_management_service.py:402`
...
- create_future_partitions：单月失败仍返回已成功月份列表（并最终抛 DatabaseError）
