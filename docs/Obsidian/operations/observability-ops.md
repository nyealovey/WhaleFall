...
- 先记录 `message_code` 与 `message`, 判断是 "业务失败" 还是 "异常"(见 [[standards/backend/hard/action-endpoint-failure-semantics]]).
...
5) 回到代码:
   ...
   - 数据一致性: 对照 [[standards/backend/hard/write-operation-boundary]] 判断 commit/rollback 预期是否正确.
...
3) 如果出现重复执行/漏执行:
   ...
   - 检查任务是否在 `app.app_context()` 内运行(见 [[standards/backend/hard/task-and-scheduler]]).
...
- 后端是否返回标准 envelope 与 `message_code`(见 [[standards/backend/hard/error-message-schema-unification]]).
