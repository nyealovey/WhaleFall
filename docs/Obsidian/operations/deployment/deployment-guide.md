---
title: 部署 Runbook (Docker Compose)
aliases:
  - deployment-guide
tags:
  - operations
...
> [!summary] TL;DR
> - 启动: `make prod deploy` (或 `docker compose -f docker-compose.prod.yml up -d --build`).
> - DB: 空库初始化 "upgrade" 或 "sql init + stamp" 二选一, 详见 [[reference/database/schema-baseline]].
> - 验证: `docker compose -f docker-compose.prod.yml ps` + health checks.
...
> 本仓库同时存在两种用法:
> - `scripts/deploy/*` 使用 `docker compose` (v2).
> - `Makefile.prod` 使用 `docker-compose` (v1).
> 如果你的机器只有一种, 请安装另一种或做别名, 例如 `alias docker-compose='docker compose'`.
...
- 迁移与基线规则见 [[standards/backend/standard/database-migrations]] 与 [[reference/database/schema-baseline]].
- 空库初始化二选一(不要重复执行两条路径):
...
> [!tip] validate-env
> `scripts/setup/validate-env.sh` 会校验 URL 形态与必填项.
> 如失败, 优先修正 `.env`, 不要 "跳过校验继续部署".
...
按 [[reference/database/schema-baseline]] 二选一:
...
> [!note] hot update
> 若你使用了热更新脚本, 请改用 [[operations/hot-update/hot-update-guide]] 的回滚流程(脚本会强制以远端为准).
...
- 处理方式:
  - 停掉占用端口的服务.
  - 或修改 `docker-compose.prod.yml` 的端口映射后再部署.
...
- 按 [[standards/backend/standard/database-migrations]] 与 [[reference/database/schema-baseline]] 执行:
  - 空库: `upgrade` 或 `SQL init + stamp` 二选一
  - 已有库: 优先 `stamp` 对齐版本, 再 `upgrade`

### 6) 数据/日志不持久化

> [!warning] userdata volume
