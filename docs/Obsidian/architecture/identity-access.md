...
- 细节见 [[standards/backend/action-endpoint-failure-semantics]] 与 [[standards/backend/write-operation-boundary]].

## 7. 新增 endpoint 的最小 checklist

- contract: 更新 `docs/Obsidian/API/**-api-contract.md` 的 "Endpoints 总览" 表格(见 [[standards/doc/api-contract-markdown-standards]]).
...
- errors: 对外口径走统一错误封套(见 [[standards/backend/error-message-schema-unification]]).
