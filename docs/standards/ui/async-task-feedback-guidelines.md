...
> 创建：2025-12-29  
> 更新：2025-12-29  
> 范围：所有触发异步任务/批量同步/批量操作的前端入口与后端启动接口
...
- MUST NOT：在各调用点扩散 `message || error`、`success || Boolean(message)` 等互兜底链（对齐 `docs/standards/backend/error-message-schema-unification.md`）。
...
- MUST：异步任务启动接口返回标准 JSON envelope（见 `docs/standards/backend/api-response-envelope.md`）。
