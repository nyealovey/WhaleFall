...
> 创建: 2025-12-28
> 更新: 2025-12-28
> 范围: `app/api/v1/**` Flask-RESTX endpoints, 尤其是 `/actions/*`, 以及 `safe_route_call` 事务边界
> 关联:
> - `./api-response-envelope.md`
> - `./error-message-schema-unification.md`
> - `./write-operation-boundary.md`
...
- MUST: 对外错误封套必须遵循 `api-response-envelope.md`.
...
- MAY: 在 service/coordinator 内使用 `with db.session.begin_nested(): ...` 创建 savepoint, 让局部失败不影响外层事务(见 `write-operation-boundary.md`).
