...
- 文件名 MUST 使用英文 `kebab-case.md`，并建议带日期前缀：`YYYY-MM-DD_database-audit-report.md`。
...
- 每条发现 MUST 给出可定位证据（文件:行号/migration id/SQL），并包含修复建议与验证方法。
...
- 建议报告路径：`docs/reports/YYYY-MM-DD_database-audit-report.md`
...
“范围与方法”中必须包含：
- Schema 概览（文字 ERD）：核心实体/关键关系/关键约束与索引摘要
- 关键用例反推：Top 10 查询/写入路径及其依赖的表/索引/约束

“发现清单（按 P0/P1/P2）”中必须包含：
- 模型设计问题清单（每条包含：标题/证据/影响/根因/建议/验证）
- 兼容/防御/回退/适配清单（与数据结构相关，逐条按“位置/类型/描述/建议”输出）

“建议与后续行动”中必须包含：
- 缺口地图：我应该补写的建模规范/ADR 标题（例如：`主键策略（UUID vs bigserial）`、`软删除与审计字段规范`、`迁移分阶段策略（零停机）`、`索引命名与复合索引准则` 等）
- 最小可执行路线图（<=8 条行动项，按 P0→P2 排序）
