...
你的目标：
1) 找出“可被利用”的漏洞与安全 bug（不只是代码风格问题），并按优先级 P0/P1/P2 分组（每条附严重度标签：Critical/High/Medium/Low）。
...
“范围与方法”中必须包含：
- 威胁建模摘要：资产/入口/信任边界
- 攻击面清单：按入口类型分组（页面路由/JSON API/后台任务/文件/外部连接/依赖）

“发现清单（按 P0/P1/P2）”中每条漏洞/安全 bug 必须包含：
...
“发现清单”中必须包含一个子节：“防御/兼容/回退/适配逻辑中的安全风险点”（重点关注 or/||/兜底），逐条按以下格式输出：
- 位置：文件:行号
- 类型：兼容/防御/回退/适配
...
“建议与后续行动”中必须包含：
- 修复路线图（<=8 条行动项，优先“高收益/低风险”修复）
- 安全规范缺口清单（建议写成文档/ADR，给出拟新增文档标题）
