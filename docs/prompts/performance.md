...
“范围与方法”中必须包含：
- 症状分型与我需要的最小补充信息（页面慢/同步慢/API 慢：各列出 5 个数据点）
- 观测最小闭环（建议新增/检查）：日志字段与计时点/慢阈值与采样策略/DB 慢查询与索引检查手段
- 瓶颈假设 → 证据 → 修复 表格（每个慢点至少 3 个假设，按概率排序）

“发现清单（按 P0/P1/P2）”中必须包含：
- 慢点清单（每条包含：标题/证据（含现象数据）/瓶颈假设/需要的证据/修复建议/验证）
- 防御/回退路径的性能风险清单（逐条按“位置/类型/描述/建议”输出，重点关注 or/||/?? 兜底）

“建议与后续行动”中必须包含：
- 最小可执行修复路线图（最多 10 条；每条行动项都要写：预计收益（影响范围）+ 风险 + 验证方式）
