...
模块分层与职责（按目录分组）
关键入口点与调用链概览
主要依赖方向与违反点（跨层调用/循环依赖）
II. “重复与债务热力图”

Top 重复点列表（含证据：文件:行号）
重复类型归类（拷贝粘贴/同义不同名/同名不同义/散落策略/环境分支）
可删代码估算（粗略即可：行数/文件数/重复块数）
III. “公共组件抽取清单”（按 P0/P1/P2 分组；每条包含）
...
分阶段策略（最少 2 阶段）：引入兼容层 → 替换调用 → 去除旧逻辑
风险点与缓解：并行实现、行为差异、配置差异、数据结构漂移
兼容/兜底路径的埋点统计方案（命中率阈值 → 删除条件）
V. “最小可执行路线图（<=8 条行动项）”

每条包含：改动范围（目录/模块）+ 验证方式 + 预估收益（删代码/减少重复/降低风险）
严格按优先级排序：P0（本周可做）/P1（本月）/P2（长期）
约束（必须遵守）：
