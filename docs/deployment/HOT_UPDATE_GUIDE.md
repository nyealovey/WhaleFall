# 鲸落项目热更新指南

## 📋 概述

热更新脚本 `quick-update-prod-flask.sh` 专门用于快速更新Flask应用代码，无需重建容器，保留所有数据。

## 🚀 功能特点

- **代码热更新** - 直接拷贝代码到运行中的容器
- **最小停机时间** - 仅需30-60秒重启时间
- **数据完全保留** - 不重建容器，保留所有数据
- **自动验证** - 更新后自动验证服务状态
- **智能检查** - 检查依赖服务状态

## 📁 文件位置

```
scripts/deployment/quick-update-prod-flask.sh
```

## 🔧 使用方法

### 1. 基本使用

```bash
# 进入项目目录
cd /path/to/WhaleFall

# 执行热更新
./scripts/deployment/quick-update-prod-flask.sh
```

### 2. 使用前准备

确保以下服务正在运行：
- ✅ Flask容器 (`whalefall`)
- ✅ PostgreSQL数据库
- ✅ Redis缓存

### 3. 更新流程

脚本会自动执行以下步骤：

1. **检查系统要求** - 验证Docker和Docker Compose
2. **检查服务状态** - 确保Flask容器正在运行
3. **拉取最新代码** - 从Git仓库拉取最新代码
4. **拷贝代码到容器** - 将新代码拷贝到运行中的容器
5. **重启Flask服务** - 重启容器以加载新代码
6. **等待服务就绪** - 等待应用完全启动
7. **验证更新** - 执行健康检查验证更新成功

## 📊 更新内容

脚本会更新以下文件和目录：

```
app/                    # 应用代码（包括路由、模型、模板、静态资源等）
migrations/            # 数据库迁移
sql/                   # SQL脚本
docs/                  # 文档
tests/                 # 测试文件
scripts/               # 脚本文件
*.py                   # Python文件
*.md                   # Markdown文件
*.txt                  # 文本文件
*.toml                 # 配置文件
*.yml                  # YAML配置
*.yaml                 # YAML配置
*.sh                   # Shell脚本
*.ini                  # INI配置
*.lock                 # 锁定文件
```

### 常见更新场景

#### 1. 前端资源更新（模板、JS、CSS）

当更新前端资源时，热更新会自动同步：

```bash
# 更新的文件示例
app/templates/admin/partitions/index.html
app/templates/admin/partitions/charts/partitions-charts.html
app/static/js/modules/views/admin/partitions/index.js
app/static/css/pages/admin/partitions.css
```

**注意**：前端资源更新后，用户需要刷新浏览器（Ctrl+F5 强制刷新）才能看到最新效果。

#### 2. 后端代码更新（路由、模型、服务）

后端代码更新会在容器重启后立即生效：

```bash
# 更新的文件示例
app/routes/partition.py
app/models/user.py
app/services/partition_management_service.py
```

**注意**：如果修改了模型字段，可能需要执行数据库迁移。

#### 3. 数据库迁移更新

如果更新包含数据库结构变更，需要额外执行迁移：

```bash
# 热更新后执行迁移
docker exec whalefall_app_prod flask db upgrade
```

## ⚠️ 注意事项

### 前置条件

1. **Flask容器必须运行** - 如果容器未运行，脚本会报错退出
2. **依赖服务必须运行** - PostgreSQL和Redis必须正常运行
3. **Git仓库状态** - 当前目录必须是Git仓库
4. **环境变量配置** - 确保`.env`文件存在且配置正确

### 限制说明

1. **不更新系统依赖** - 仅更新应用代码，不更新Python包
2. **不更新数据库结构** - 如需数据库迁移，需单独执行
3. **不更新配置文件** - 容器内配置文件不会更新
4. **不更新Docker镜像** - 使用现有镜像，不重新构建

### 浏览器缓存问题

更新前端资源（HTML、JS、CSS）后，用户可能看不到最新效果，需要：

1. **强制刷新浏览器** - 按 `Ctrl+F5`（Windows/Linux）或 `Cmd+Shift+R`（Mac）
2. **清除浏览器缓存** - 在浏览器设置中清除缓存
3. **使用隐私模式** - 在隐私/无痕模式下测试新功能

### 数据模型变更

如果更新涉及数据模型变更（如 `app/models/user.py`），需要注意：

1. **字段添加** - 通常安全，但需要设置默认值
2. **字段删除** - 需要先执行数据库迁移
3. **字段类型变更** - 需要数据迁移脚本
4. **关系变更** - 可能影响现有数据，需要谨慎测试

**示例**：本次更新修改了 `User.to_dict()` 方法，将 `created_at` 格式从完整时间戳改为仅日期，这是安全的更新，不影响数据库结构。

## 🔍 故障排除

### 常见问题

#### 1. Flask容器未运行

```bash
# 错误信息
Flask容器未运行: Exited (1) 2 hours ago

# 解决方案
# 先启动Flask容器
docker compose -f docker-compose.prod.yml up -d whalefall
```

#### 2. 代码拷贝失败

```bash
# 检查容器状态
docker compose -f docker-compose.prod.yml ps whalefall

# 检查容器日志
docker compose -f docker-compose.prod.yml logs whalefall
```

#### 3. 健康检查失败

```bash
# 手动检查健康状态
curl http://localhost:5001/health/api/basic

# 检查应用日志
docker compose -f docker-compose.prod.yml logs whalefall --tail 50
```

### 手动回滚

如果更新后出现问题，可以手动回滚：

```bash
# 1. 停止当前容器
docker compose -f docker-compose.prod.yml stop whalefall

# 2. 回滚到上一个版本
git reset --hard HEAD~1

# 3. 重新拷贝代码
./scripts/deployment/quick-update-prod-flask.sh
```

## 📈 性能优化

### 更新频率建议

- **开发环境** - 可以频繁更新
- **测试环境** - 建议每日更新
- **生产环境** - 建议在维护窗口更新

### 监控建议

更新后建议监控：

```bash
# 检查容器资源使用
docker stats whalefall_app_prod

# 检查应用日志
docker compose -f docker-compose.prod.yml logs -f whalefall

# 检查健康状态
curl http://localhost:5001/health/api/basic
```

### 更新后验证清单

完成热更新后，建议执行以下验证：

- [ ] 访问主页，确认应用正常响应
- [ ] 测试登录功能
- [ ] 检查更新的页面/功能是否正常
- [ ] 查看应用日志，确认无错误
- [ ] 测试关键业务流程
- [ ] 强制刷新浏览器，确认前端资源已更新

**本次更新验证示例**：
```bash
# 1. 访问用户管理页面
curl -I http://localhost:5001/users/

# 2. 检查创建时间格式（应为 YYYY-MM-DD）
# 登录后访问用户管理页面，查看创建时间列

# 3. 访问分区管理页面
curl -I http://localhost:5001/partition/

# 4. 检查统计卡片和图表是否正常显示
# 登录后访问分区管理页面，确认卡片和图表正常
```

## 🔧 高级用法

### 1. 自定义更新内容

如果需要只更新特定文件，可以修改脚本中的拷贝部分：

```bash
# 只更新应用代码
cp -r app "$temp_dir/"

# 只更新特定文件
cp app/routes/auth.py "$temp_dir/app/routes/"
```

### 2. 更新前备份

建议在更新前备份重要数据：

```bash
# 备份数据库
docker exec postgres pg_dump -U postgres taifish > backup_$(date +%Y%m%d_%H%M%S).sql

# 备份应用代码
tar -czf app_backup_$(date +%Y%m%d_%H%M%S).tar.gz app/
```

### 3. 批量更新

可以创建批量更新脚本：

```bash
#!/bin/bash
# 批量更新多个环境

environments=("dev" "test" "prod")
for env in "${environments[@]}"; do
    echo "更新 $env 环境..."
    cd "/path/to/$env/WhaleFall"
    ./scripts/deployment/quick-update-prod-flask.sh
done
```

## 📞 技术支持

如果遇到问题，请：

1. 检查容器状态和日志
2. 验证网络连接
3. 确认文件权限
4. 联系技术支持团队

---

**注意**: 本脚本专为生产环境设计，使用前请确保充分测试。
