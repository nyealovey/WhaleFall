...
## 背景
- 标签（`app/models/tag.py` + `instances.tags`）与账户分类（`app/models/account_classification.py`）允许用户自定义颜色，但实际使用中出现大量 HEX/RGB 颜色，导致页面视觉噪声，与《界面色彩与视觉疲劳控制指南》（`docs/standards/color-guidelines.md`）“禁止硬编码颜色、语义色 ≤4”冲突。
...
## 实施步骤
1. **定义色盘**
   - 更新 `app/static/css/variables.css` 并补充 `docs/standards/color-guidelines.md` 的“引用示例”章节。
2. **数据迁移**
...
## 文档与沟通
- 将本方案链接至 `docs/refactor/accounts_ledger_color_refactor.md` 的“后续工作”章节，明确标签/分类配色统一是下一阶段目标。
