...
## 背景
- 当前“变更历史”弹窗重用早期自定义 UI：纯橙顶栏、左右圆角与 `badge` 条框，整体使用 HEX 实底色（`#ff5a00`、`#8fd19e`）+ 白底内容，严重偏离《界面色彩与视觉疲劳控制指南》（`docs/standards/color-guidelines.md`）对 2-3-4 规则及组件化要求。
...
- 在 `docs/standards/color-guidelines.md` 的“实施步骤”中增加“历史弹窗案例”条目，提醒所有日志类弹窗引用同一套组件与色彩控制。
