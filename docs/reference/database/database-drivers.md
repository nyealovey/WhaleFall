...
> 创建：2025-09-07  
> 更新：2025-12-26  
> 范围：主数据库（SQLAlchemy）与外部实例连接（连接测试/同步）  
> 关联：`../config/environment-variables.md`；`./database-permissions-overview.md`
...
> 备注：仓库依赖中包含 `pyodbc>=5.2.0`，但当前连接适配器未使用（SQL Server 以 `pymssql` 为准）。
