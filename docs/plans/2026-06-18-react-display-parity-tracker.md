...
| 实例/数据库台账/账户台账 | 旧版标签筛选使用共享“选择标签”弹窗，支持搜索、分类导航、多选托盘和确认写回；React 早期改成单个 Select | 已补共享 shadcn Dialog `TagSelectorFilter`，三页标签筛选均恢复旧版同等多选能力 | Done |
...
| `/console/instances` | 独立筛选区、实例列表、批量操作、实例详情入口；标签筛选使用共享“选择标签”弹窗 | 新版曾有详情弹窗和详情页两个入口，并有额外说明/每页条数；2026-06-25 复核仍发现 `RESOURCE INVENTORY` 残留；2026-06-26 发现标签筛选被简化为单选 Select | 已删除列表详情弹窗入口，只保留一个 `/console/instances/{id}` 详情页入口；列表英文 eyebrow 和说明文字已继续收敛；标签筛选已恢复为可搜索、按分类过滤、多选确认的共享 Dialog | Done |
...
| `/console/database-ledgers` | 数据库台账列表、筛选、同步、导出、表容量详情；标签筛选使用共享“选择标签”弹窗 | 新版主要差异为筛选位置和说明文字；2026-06-25 复核仍发现 `Database ledger` 残留；2026-06-26 发现标签筛选被简化为单选 Select | 字段、筛选和动作已核对；删除旧版没有的 `每页 X 条` 辅助说明；英文 eyebrow 和说明文字已继续收敛；标签筛选已恢复为共享 Dialog | Done |
| `/console/account-ledgers` | 账户台账列表、筛选、同步、导出、权限/变更历史；标签筛选使用共享“选择标签”弹窗 | 新版主要差异为筛选位置和说明文字；2026-06-26 发现标签筛选被简化为单选 Select | 字段、筛选和动作已核对；删除旧版没有的 `每页 X 条` 辅助说明；标签筛选已恢复为共享 Dialog | Done |
...
| 标签筛选 | 实例、数据库台账、账户台账恢复旧版共用“选择标签”弹窗 | 能搜索标签名称/代码/分类，按分类导航，多选标签，确认后写回当前列表筛选 |
