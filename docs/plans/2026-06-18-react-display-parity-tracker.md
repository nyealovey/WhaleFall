...
| 范围 | 差异 | 处理方式 | 状态 |
| --- | --- | --- | --- |
| 所有 React 页面 | 新版 PageHeader 通常包含英文 eyebrow 和页面说明，旧版多数页面没有 | 逐页删减；只保留能帮助识别页面的中文标题和旧版必要内容 | Open |
...
| --- | --- | --- | --- | --- |
| `/console/dashboard` | 4 个核心指标、风险告警、错误和告警日志趋势、系统状态、运行时间 | 新版额外展示账户分类、日志等级图、同步趋势图、容量利用率标记和说明文字 | 删除旧版没有的账户分类、日志等级图、同步趋势图；容量利用率标记按旧版容量展示口径复核后决定是否保留 | Open |
...
| `/console/clusters` | SQL Server/MySQL 标签切换；新增/编辑内含基本信息、绑定实例、AG 配置 | 新版已恢复标签切换，但绑定实例和 AG 维护放在列表下方同页区域 | 评估收回到新增/编辑流程；避免详情弹窗继续发起子弹窗；保留单页面只显示一种群集 | Open |
...
| `/console/settings` | 设置模块导航；一次只显示告警、JumpServer、风险规则、Veeam、AD 中的一个面板 | 新版纵向展示多个设置区，并增加启用配置/风险规则/AD/Veeam 指标卡 | 改回单面板切换；删除顶部指标卡；保留旧版设置字段和操作入口 | Open |
