...
| `/console/instances/:id` | 实例基础信息、账户信息、AG 账户、容量、审计、备份等详情块；旧版详情内容在标签框内切换 | 新版早期缺实例 ID、版本、标签、详情页编辑/回收站、账户/容量汇总、账户操作和备份字段；审计和备份曾独立显示在页签框外 | 已删除原始实例 JSON 和旧版没有的说明头；补齐实例 ID、数据库版本、标签、编辑、移入回收站、账户/容量四项汇总、查看权限、变更历史、表容量入口、Backup ID、覆盖数量、平台、数据大小、备份大小和压缩率；账户、AG 账户、容量、审计、备份统一在同一个 shadcn Tabs 卡片中切换 | Done |
