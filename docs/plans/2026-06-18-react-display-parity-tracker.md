...
- 复核对象包括非路由入口：旧版共享模板、页脚、全局 Shell、批量页、行内操作、表单内选择器和确认弹窗都必须纳入清单。
...
| 实例/数据库台账/账户台账 | 旧版标签筛选使用共享“选择标签”弹窗，支持搜索、分类导航、多选托盘和确认写回；React 早期改成单个 Select | 已补共享 shadcn Dialog `TagSelectorFilter`，三页标签筛选均恢复旧版同等多选能力；2026-06-26 复核补齐“应用”和“重置”，重置会清空当前标签筛选 | Done |
...
| `/console/instances` | 独立筛选区、实例列表、批量操作、实例详情入口；标签筛选使用共享“选择标签”弹窗 | 新版曾有详情弹窗和详情页两个入口，并有额外说明/每页条数；2026-06-25 复核仍发现 `RESOURCE INVENTORY` 残留；2026-06-26 发现标签筛选被简化为单选 Select；复核又发现已选标签无法一键清空 | 已删除列表详情弹窗入口，只保留一个 `/console/instances/{id}` 详情页入口；列表英文 eyebrow 和说明文字已继续收敛；标签筛选已恢复为可搜索、按分类过滤、多选应用/重置的共享 Dialog | Done |
...
| `/console/database-ledgers` | 数据库台账列表、筛选、同步、导出、表容量详情；标签筛选使用共享“选择标签”弹窗 | 新版主要差异为筛选位置和说明文字；2026-06-25 复核仍发现 `Database ledger` 残留；2026-06-26 发现标签筛选被简化为单选 Select；复核又发现已选标签无法一键清空 | 字段、筛选和动作已核对；删除旧版没有的 `每页 X 条` 辅助说明；英文 eyebrow 和说明文字已继续收敛；标签筛选已恢复为带应用/重置的共享 Dialog | Done |
| `/console/account-ledgers` | 账户台账列表、筛选、同步、导出、权限/变更历史；标签筛选使用共享“选择标签”弹窗 | 新版主要差异为筛选位置和说明文字；2026-06-26 发现标签筛选被简化为单选 Select；复核又发现已选标签无法一键清空 | 字段、筛选和动作已核对；删除旧版没有的 `每页 X 条` 辅助说明；标签筛选已恢复为带应用/重置的共享 Dialog | Done |
...
| `/console/tags` | 四项标签指标、标签列表、筛选、批量分配入口 | 新版标签辅助指标口径不完整，并增加分类说明和查看详情入口；批量分配曾被做成弹窗 | 已恢复均值/分类、启用数、停用数、启用/分类及一位小数百分比；删除分类说明和详情入口；批量分配入口改为跳转独立页面 | Done |
| `/console/tags/bulk/assign` | 独立批量页面，包含分配模式/移除模式、实例选择、标签选择、当前选择、清空选择、执行进度 | React 曾在标签管理页内用弹窗承载，并额外加入旧版没有的“批量移除指定标签”模式 | 已恢复独立页面工作台；保留分配模式和移除模式两种旧版模式，移除模式隐藏标签选择并移除所选实例上的所有标签 | Done |
| `/console/partitions` | 两个命令、四项指标、核心趋势、分区列表；核心趋势按日/周/月/季展示四条旧版指标线：实例数、实例统计数量、数据库数、数据库统计数量；创建和清理参数位于弹窗；旧版表头为分区名称、表类型、大小、记录数、分区月份、状态 | 新版把分区日期和保留月份提前到工具栏，并增加图表状态/时间和分页说明；2026-06-25 复核发现表头语义被合并；2026-06-26 复核发现核心趋势只渲染首条 dataset，且周/月/季按 28/90/365 请求导致不是旧版“最近 7 个周期”口径 | 已把创建年月与保留月数移回 shadcn Dialog/AlertDialog；删除图表额外状态/时间和分页说明；分区列表表头恢复旧版展示语义；核心趋势改为全宽 Recharts LineChart，渲染 v1 返回的全部 datasets，并统一按 7 个周期请求日/周/月/季 | Done |
...
| 范围 | 任务 | 验收 |
| --- | --- | --- |
| 审计方法 | 把共享模板、页脚、批量页、工具栏选择器、行内弹窗和确认弹窗纳入迁移范围 | 后续不再只按菜单路由判断迁移完成 |
| 标签筛选 | 实例、数据库台账、账户台账恢复旧版共用“选择标签”弹窗 | 能搜索标签名称/代码/分类，按分类导航，多选标签，应用写回当前列表筛选，重置清空当前标签筛选 |
| 全局搜索/筛选 | 所有 DataTable 和自定义筛选区统一补操作按钮 | 搜索/筛选继续即时生效；页面展示“应用筛选”和“重置”，重置清空搜索、下拉筛选及标签/时间范围等额外筛选 |
| 容量筛选 | 实例容量、数据库容量恢复旧版逐级联动 | 数据库类型固定包含 MySQL、PostgreSQL、SQL Server、Oracle；实例按数据库类型加载，数据库按实例加载，上级变化清空下级 |

### Batch 9: 标签批量页体验回退

| 范围 | 任务 | 验收 |
| --- | --- | --- |
| 标签批量分配/移除 | 从标签管理弹窗恢复为 `/console/tags/bulk/assign` 独立页面 | 页面包含返回入口、分配模式/移除模式、实例分组、标签分组、当前选择、清空选择和执行按钮；移除模式不展示标签选择 |

### Batch 10: 分区核心趋势复核修复

| 范围 | 任务 | 验收 |
| --- | --- | --- |
| 分区核心指标图 | 恢复旧版四条指标线和最近 7 个周期口径 | 日/周/月/季标题、副标题和请求参数与旧版一致；图表全宽展示实例数、实例统计数量、数据库数和数据库统计数量四组数据 |
...
2026-06-26 Batch 8 非路由共享入口补漏验证：
...
2026-06-26 Batch 9 标签批量页体验回退验证：
...
2026-06-26 Batch 10 分区核心指标趋势复核修复验证：
...
- 2026-06-26: 修正迁移复核方法，补入非路由共享入口；实例管理、数据库台账、账户台账的标签筛选从单选 Select 恢复为旧版共用“选择标签”多选弹窗。
- 2026-06-26: 标签批量分配/移除恢复旧版独立页面形态，删除 React 新增的弹窗式批量入口和“批量移除指定标签”模式。
- 2026-06-26: 分区核心指标趋势恢复旧版四指标线、全宽图表和最近 7 个周期参数口径。
- 2026-06-26: 全量排查共享“选择标签”筛选框，确认仅实例管理、数据库台账、账户台账三处复用；三处均补“应用”和“重置”，重置会清空外层标签筛选参数。
- 2026-06-26: 全量排查 DataTable 与自定义搜索/筛选区，保留原即时生效交互，统一补“应用筛选”和“重置”；实例、数据库台账、账户台账、日志、账户变更、会话、用户、凭据、标签、群集、分区等服务端表格重置均接入完整 state reset。
- 2026-06-26: 修复容量页筛选来源和联动口径，数据库类型不再依赖当前容量数据，实例和数据库下拉恢复旧版逐级锁定，避免跨数据库类型混入。
- 2026-06-26: 新建 `2026-06-26-react-functional-parity-audit.md`，从线上 React/旧版页面、旧版模板 include 和 React 弹窗代码三条线重新记录功能差异；P0 先跟踪凭据类型/密码显隐、调度重初始化与删除入口、审计统计窗口硬编码、容量筛选多选口径。
- 2026-06-29: 按功能一致性审计完成 P0/P1 本地修复：凭据表单、调度可见动作、审计统计时间窗口、容量多选联动、标签批量实例展示、实例详情已删除开关和风险中心文案均已补齐；待重新构建部署后线上复核。
