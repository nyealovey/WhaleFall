...
- 已接入页面: 22
- 仍为占位页: 0
- 当前策略: 按旧版内容 parity 清单逐页返工，优先补齐字段、筛选、图表、详情和操作入口
- 旧站点状态: 保持不动，作为回滚入口
...
| `/console/instances` | `/instances/` | Parity - content | `/api/v1/instances?page=1&limit=20` | 已按旧版补齐名称、类型、主机/IP、状态、审计、已托管、备份、活跃、版本/同步、标签、操作列；搜索、类型、状态、审计、托管、备份、标签筛选由 DataTable 工具栏承载；新增、批量删除、批量测试、批量导入、详情和连接测试 API 待迁移 |
| `/console/database-ledgers` | `/databases/ledgers` | Parity - content | `/api/v1/databases/ledgers?page=1&limit=20` | 已按旧版补齐数据库/实例、类型、数据库大小、标签、操作列；搜索、类型、标签筛选由 DataTable 工具栏承载；同步所有数据库、导出、容量趋势跳转 API 待迁移 |
| `/console/account-ledgers` | `/accounts/ledgers` | Parity - content | `/api/v1/accounts/ledgers?page=1&limit=20` | 已按旧版补齐账户/实例、是否可用、是否删除、是否超管、AD 状态、分类、类型、标签、操作列；搜索、分类、AD 状态、标签筛选由 DataTable 工具栏承载；同步所有账户、导出、权限详情 API 待迁移 |
| `/console/capacity/instances` | `/capacity/instances` | Parity - content | `/api/v1/capacity/instances?period_type=daily&page=1&limit=20&start_date=...&end_date=...`, `/api/v1/capacity/instances/summary?period_type=daily&start_date=...&end_date=...` | 已按旧版补齐刷新数据、统计当前周期、数据库类型/实例/周期筛选、在线实例数/总容量/平均容量/最大容量指标，以及容量统计趋势图、容量变化趋势图、容量变化趋势图(百分比)；刷新与统计计算 API 待迁移 |
| `/console/capacity/databases` | `/capacity/databases` | Parity - content | `/api/v1/capacity/databases?period_type=daily&page=1&limit=20&start_date=...&end_date=...`, `/api/v1/capacity/databases/summary?period_type=daily&start_date=...&end_date=...` | 已按旧版补齐刷新数据、统计当前周期、数据库类型/实例/数据库/周期筛选、总数据库数/总容量/平均容量/最大容量指标，以及容量统计趋势图、容量变化趋势图、容量变化趋势图(百分比)；刷新与统计计算 API 待迁移 |
...
| `/console/account-classifications` | `/accounts/classifications/` | Parity - content | `/api/v1/accounts/classifications`, `/api/v1/accounts/classifications/rules` | 已按旧版补齐自动分类入口、账户分类面板、规则管理面板、风险等级、系统标记、规则分组、命中数和查看/编辑/删除入口；分类/规则 CRUD 与自动分类 API 待迁移 |
...
| `/console/scheduler` | `/scheduler/` | Parity - content | `/api/v1/scheduler/jobs` | 已按旧版补齐运行中/已暂停任务分组、任务名称、状态、下次运行、上次运行、任务 ID、触发器参数和重新初始化/暂停/恢复/立即执行/编辑/删除入口；写操作 API 待迁移 |
| `/console/sync-sessions` | `/history/sessions/` | Parity - content | `/api/v1/sync-sessions?page=1&limit=20` | 已按旧版补齐运行ID、状态、进度、任务、来源、分类、开始时间、耗时、操作列；来源、分类、状态筛选由 DataTable 工具栏承载；详情和取消 API 待迁移 |
...
| `/console/settings` | `/admin/system-settings` | Parity - content | `/api/v1/alerts/email-settings`, `/api/v1/risk-center/rules`, `/api/v1/integrations/jumpserver/source`, `/api/v1/integrations/veeam/sources`, `/api/v1/ad-domain-configs` | 已按旧版补齐设置模块导航、告警发送/规则设置、风险规则、JumpServer、Veeam、AD 设置表单字段和操作入口；保存、测试、同步、删除等写操作 API 待迁移 |
...
| `/console/partitions` | `/partition/` | Parity - content | `/api/v1/partitions/status`, `/api/v1/partitions?page=1&limit=20`, `/api/v1/partitions/core-metrics?period_type=daily&days=7` | 已按旧版补齐创建分区/清理旧分区入口、分区总数/总大小/总记录数/健康状态指标、核心指标趋势、日周月季切换外观和分区列表；创建、清理和周期联动 API 待迁移 |

## 待迁移

当前导航内剩余页面的只读首屏已全部迁移到 React；接下来按完整替代口径逐页返工。新增、编辑、删除、同步、导出、批量绑定、清理等写操作仍需后续按风险分批迁移。
...
```bash
npm --prefix frontend run test       # 21 files, 79 tests passed
npm --prefix frontend run typecheck  # passed
npm --prefix frontend run lint       # passed
npm --prefix frontend run build      # passed; /static/css/fonts.css remains runtime-resolved
git diff --check                     # passed
