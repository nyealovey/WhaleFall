...
# 问题：文件路径需要修改两次！
# 第一次：capacity_stats/chart_renderer.js → capacity_stats/chart-renderer.js
# 第二次：capacity_stats/chart-renderer.js → capacity-stats/chart-renderer.js
```
...
# 目录内的文件自动跟随移动：
# capacity_stats/chart_renderer.js → capacity-stats/chart_renderer.js
...
✅ 目录已重命名
✅ 目录内的文件自动跟随移动
✅ 文件路径：capacity_stats/chart_renderer.js → capacity-stats/chart_renderer.js
```
...
`scripts/refactor_naming.sh` 已经按照正确的顺序执行：
...
- [完整重构清单](./完整重构清单.md) - 所有重构任务
- [重构优先级清单](./重构优先级清单.md) - 快速参考
- [目录重构补充说明](./目录重构补充说明.md) - 目录重构详情

---

*遵循正确的执行顺序，让重构更高效、更安全！*
