...
| P2 | `accounts/account_classification.js`、`admin/scheduler.js`、`unified_search.js`、`tags/batch_assign.js`、`accounts/list.js` | 待梳理的复杂表单 |
| P3 | 低频或查询类表单 | 依赖上阶段经验，逐步统一 |

> 每个阶段结束后更新基线脚本和文档，确保后续页面按新套路实现。
>
> **当前进展**：P0（凭据创建/编辑、登录）与 P1（实例创建/编辑、修改密码）已完成；标签创建/编辑、账户分类（分类/规则）与统一搜索表单已接入统一校验，下一步聚焦标签批量分配等剩余 P2 场景。
