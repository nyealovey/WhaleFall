...
-- 4. 授予information_schema数据库查询权限（用于数据库大小监控）
...
SELECT 'information_schema测试' AS 测试项目, COUNT(*) AS 可访问的数据库数量 FROM information_schema.tables WHERE table_schema NOT IN ('information_schema', 'performance_schema', 'mysql', 'sys');
