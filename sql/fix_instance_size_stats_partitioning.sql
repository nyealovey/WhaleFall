-- 修复 instance_size_stats 表的分区问题
-- 如果表已经存在但不是分区表，需要重新创建

-- 检查表是否存在并创建/修复
DO $$
BEGIN
    -- 如果表存在但不是分区表，删除并重新创建
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'instance_size_stats') THEN
        -- 检查是否是分区表
        IF NOT EXISTS (
            SELECT 1 FROM pg_class 
            WHERE relname = 'instance_size_stats' 
            AND relkind = 'p'
        ) THEN
            RAISE NOTICE 'Table instance_size_stats exists but is not partitioned. Dropping and recreating...';
            ...
            RAISE NOTICE 'Table instance_size_stats recreated as partitioned table.';
        ELSE
            RAISE NOTICE 'Table instance_size_stats is already partitioned.';
        END IF;
    ELSE
        RAISE NOTICE 'Table instance_size_stats does not exist. Creating new partitioned table...';
        ...
        RAISE NOTICE 'Table instance_size_stats created as partitioned table.';
    END IF;
END $$;
