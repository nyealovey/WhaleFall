...
BASELINE_FILE="${ROOT_DIR}/scripts/ci/baselines/inline-px-style-guard.txt"
...
PATTERN='style="[^"]*(height|width)\s*:\s*[0-9]+px'
...
("${RG_BIN}" -n -S -- "${PATTERN}" "${TARGETS[@]}" || true) | LC_ALL=C sort >"${current_file}"
...
if [[ ! -f "${BASELINE_FILE}" ]]; then
  ...
  echo "请先运行：./scripts/ci/inline-px-style-guard.sh --update-baseline" >&2
  exit 1
fi
...
if [[ -n "${new_hits}" ]]; then
  ...
  echo "${new_hits}" >&2
  echo "请迁移为 Token + class（参考 docs/standards/ui/layout-sizing-guidelines.md）。" >&2
  exit 1
fi
