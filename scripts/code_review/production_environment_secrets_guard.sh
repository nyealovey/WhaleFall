...
if ! command -v "${RG_BIN}" >/dev/null 2>&1; then
  echo "未检测到 rg 命令，无法执行 env.example 密钥门禁检查。" >&2
  exit 1
fi
...
SENSITIVE_VALUE_PATTERN='^(POSTGRES_PASSWORD|REDIS_PASSWORD|SECRET_KEY|JWT_SECRET_KEY|PASSWORD_ENCRYPTION_KEY)=.*\S'

if "${RG_BIN}" -n "${SENSITIVE_VALUE_PATTERN}" "${TARGET_FILE}"; then
  echo "" >&2
