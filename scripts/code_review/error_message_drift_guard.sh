...
PATTERN="\\b\\w+\\.get\\(\\s*['\\\"]error['\\\"][^\\)]*\\)\\s*or\\s*\\w+\\.get\\(\\s*['\\\"]message['\\\"][^\\)]*\\)|\\b\\w+\\.get\\(\\s*['\\\"]message['\\\"][^\\)]*\\)\\s*or\\s*\\w+\\.get\\(\\s*['\\\"]error['\\\"][^\\)]*\\)"
...
("${RG_BIN}" -n --hidden --type py -P "${PATTERN}" "${TARGET_DIR}" || true) | LC_ALL=C sort >"${current_file}"
...
if [[ -n "${new_hits}" ]]; then
  ...
  echo "${new_hits}" >&2
  echo "请按 docs/refactor/backend_error_message_schema_unification.md 统一结果结构，避免在读端写互兜底链。" >&2
  exit 1
fi
