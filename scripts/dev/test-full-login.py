    ...
    if response.status_code == 302:
        ...
        if redirect_url:
            ...
            if response.status_code == 200:
                ...
                # 4. 测试admin/app-info接口
                print("4. 测试admin/app-info接口...")
                response = session.get("http://localhost:5001/admin/app-info")
                print(f"   状态码: {response.status_code}")
                
                if response.status_code == 200:
                    print("✅ admin/app-info接口正常")
                    print(f"   响应: {response.text}")
                else:
                    print(f"❌ admin/app-info接口失败: {response.status_code}")
                
                # 5. 测试dashboard页面
                print("5. 测试dashboard页面...")
                ...
                if response.status_code == 200:
                    ...
                    print(f"❌ dashboard页面失败: {response.status_code}")
