#!/usr/bin/env python3
"""
修复的SQLite到PostgreSQL数据迁移脚本 V2
正确处理数据类型转换，特别是布尔值和JSON字段
"""
...
def process_row_data(columns: List[str], row: Tuple) -> List[Any]:
    """处理行数据，进行类型转换"""
    processed_row = []
    ...
    for i, value in enumerate(row):
        if value is None:
            processed_row.append(None)
                          ...
                          'type_specific', 'metadata', 'extra_data', 'tags']:
            # JSON字段
        ...
        elif 'time' in columns[i] or 'created_at' in columns[i] or 'updated_at' in columns[i] or 'deleted_at' in columns[i] or 'last_' in columns[i]:
            # 时间字段
...
def migrate_table_data(table_name: str, columns: List[str], data: List[Tuple]) -> int:
    ...
    try:
        ...
        for row in data:
            try:
                # 处理每一行数据
                processed_row = process_row_data(columns, row)
...
def clear_postgresql_tables():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print("PostgreSQL表数据清空完成")
        
    except Exception as e:
        print(f"清空PostgreSQL表数据失败: {e}")

def main():
    print("开始修复的SQLite到PostgreSQL数据迁移 V2...")
