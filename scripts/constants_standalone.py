...
class ConstantsAnalyzer:
    ...
    def analyze_constants_usage(self) -> None:
        """分析常量使用情况"""
    ...
    def _get_constant_classes(self) -> List[str]:
        """获取所有常量类名"""
        return [
            "SystemConstants",
            "DefaultConfig", 
    ...
    def _get_class_constants(self, constant_class: str) -> List[str]:
        """获取类中的所有常量"""
        constants = []
        
        # 从constants.py文件中解析常量
        ...
        try:
            ...
            # 查找类定义
            class_pattern = rf"class {constant_class}:.*?(?=class|\Z)"
            class_match = re.search(class_pattern, content, re.DOTALL)
            
            if class_match:
                class_content = class_match.group(0)
                # 查找常量定义
                constant_pattern = r"^\s*([A-Z_][A-Z0-9_]*)\s*="
                matches = re.findall(constant_pattern, class_content, re.MULTILINE)
                constants.extend(matches)
        
        except Exception as e:
            print(f"解析常量类 {constant_class} 时出错: {e}")
        
        return constants

    def _is_constant_used(self, content: str, constant_class: str, constant_name: str) -> bool:
        """检查常量是否被使用"""
        # 检查直接使用
        patterns = [
            f"{constant_class}.{constant_name}",
    ...
    def collect_constants_definitions(self) -> None:
        """收集常量定义"""
        constants_file = os.path.join(self.app_dir, "constants.py")
    ...
    def generate_documentation(self) -> str:
        ...
        # 收集常量定义
        self.collect_constants_definitions()
...
"""
    ...
    def _build_class_documentation(self, constant_class: str) -> str:
        """构建单个常量类的文档"""
    ...
    def save_doc(self, output_file: str = None) -> str:
        """保存文档到文件"""
        if not output_file:
            output_file = os.path.join(self.project_root, "docs", "constants", "CONSTANTS_DOCUMENTATION.md")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="鲸落独立常量管理工具")
    parser.add_argument(
        "command",
        choices=["generate-doc", "monitor", "full-analysis"],
        help="要执行的命令"
    )
    parser.add_argument(
        "--output-dir",
    ...
    # 创建常量分析器
    analyzer = ConstantsAnalyzer(args.project_root)
    
    try:
        if args.command == "generate-doc":
            print("📝 生成常量文档...")
            doc_file = analyzer.save_doc()
            print(f"✅ 文档已生成: {doc_file}")
            
        elif args.command == "monitor":
            print("📊 分析常量使用情况...")
            usage_results = analyzer.generate_usage_report()
            
            print(f"\n📈 使用统计:")
            print(f"总常量数: {usage_results['total_constants']}")
            print(f"使用文件数: {usage_results['total_files']}")
            print(f"高频使用常量: {len(usage_results['high_usage_constants'])} 个")
            print(f"未使用常量: {len(usage_results['unused_constants'])} 个")
            ...
            if usage_results['unused_constants']:
                print(f"\n⚠️  未使用常量:")
                for constant_name in usage_results['unused_constants']:
                    print(f"  - {constant_name}")
            ...
            # 保存使用报告
            report_file = os.path.join(args.output_dir, "usage_report.json")
            os.makedirs(os.path.dirname(report_file), exist_ok=True)
            
            with open(report_file, "w", encoding="utf-8") as f:
                json.dump(usage_results, f, indent=2, ensure_ascii=False, default=str)
            
            print(f"✅ 使用报告已保存: {report_file}")
