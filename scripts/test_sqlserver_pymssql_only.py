#!/usr/bin/env python3
"""
SQL Server pymssql专用测试脚本
验证只使用pymssql驱动后的权限查询功能
"""
...
def test_sqlserver_pymssql_only(instance_id: int):
    """测试只使用pymssql的SQL Server连接和权限查询"""
    ...
    print(f"🔍 测试SQL Server实例 (仅pymssql): {instance.name}")
    ...
    try:
                ...
                if users:
                    ...
                    sql = f"""
                        ...
                        WHERE grantee_principal_id = (
                            SELECT principal_id
                            FROM [{test_db}].sys.database_principals
                            WHERE name = %s
                        )
                        AND state = 'G'
                    """
        ...
        print("\n" + "="*50)
        print("🎉 pymssql专用测试完成！")
        print("如果所有测试都通过，说明SQL Server权限查询功能正常")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python test_sqlserver_pymssql_only.py <instance_id>")
        print("示例: python test_sqlserver_pymssql_only.py 1")
        sys.exit(1)
    
    instance_id = int(sys.argv[1])
    test_sqlserver_pymssql_only(instance_id)
