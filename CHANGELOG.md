...
### ✨ 新功能
- 新增容量同步修复脚本，支持一键回放指定月份的容量聚合数据
- 新增容量统计(数据库)页面，提供数据库维度的容量趋势与变化分析
- 图表新增数据库标签与实例归属提示，支持 TOP 切换与周期联动

### 🔧 改进
- 优化容量趋势图样式：突出零轴、统一柱状图透明度，提升可读性
- 搜索筛选组件样式与数据库页面对齐，去除浮动阴影与多余圆角
- 分区管理仅允许创建当前及未来月份分区，避免误创建历史分区

### 🐛 修复
- 修复仪表板统计在事务失败后无法恢复的问题
- 修复分区索引创建时引用不存在字段导致的创建失败

## [1.1.0] - 2025-09-25

### 🐛 修复
- 修复自动分类功能中的权限规则匹配BUG
- 修复标签管理页面Jinja2模板语法错误，解决页面访问异常
- 修复JavaScript重复加载问题，解决类重复声明错误
- 修复批量分配标签页面的实例显示布局问题
- 修复模板结构不完整导致的页面渲染异常

### 🔧 改进
- 优化标签管理页面JavaScript加载机制，移除不必要的组件包含
- 优化批量分配标签页面的实例显示布局，将名称和IP地址放在同一行
- 优化实例选择容器高度，确保至少可以同时展示10个数据库实例
- 改进模板结构，确保所有block都有对应的endblock标签
- 优化页面加载性能，减少不必要的JavaScript文件加载

### ✨ 新功能
- 新增实例信息紧凑显示，使用等宽字体显示IP地址
- 新增重复执行保护机制，防止页面重复初始化
- 新增模板语法验证，确保Jinja2模板结构正确
