...
> **命名说明**  
> 项目中已存在 “权限查看” 模块（负责展示权限详情）。为避免混淆，本次重构的产物统一命名为 **“权限策略配置中心”**（英文标识 `permission-policy-center`），所有文档与代码中的旧称 “权限配置模块” 均改为新名称。

将 `account_classification.js` 中的权限策略配置相关逻辑抽离到独立模块，实现以下目标：

1. **代码复用**：权限策略配置逻辑可在多个页面快速接入。
2. **集中维护**：权限相关代码统一放置，减少跨文件耦合。
3. **扩展友好**：新增数据库类型时，仅需提供对应策略实现。

## 二、当前问题

### 2.1 权限配置逻辑分散

在 `account_classification.js` 中，权限策略配置代码散落在多个函数中：
...
├── permission-policy-center.js     # 权限策略配置中心（新增）
└── permission-modal.js             # 权限查看模态框（已存在，保持原名以示区分）
```
...
```javascript
const PERMISSION_POLICY_STRATEGIES = {
...
function getPermissionPolicyStrategy(dbType) {
    const StrategyClass = PERMISSION_POLICY_STRATEGIES[dbType];
    if (!StrategyClass) {
...
#### 3.3.1 PermissionPolicyCenter 主类
...
async function loadPermissions(prefix = '') {
    ...
    await PermissionPolicyCenter.load(dbType, containerId, prefix);
}
```
...
function submitCreateRule(form) {
    ...
    // 收集选中的权限
    const selectedPermissions = PermissionPolicyCenter.collectSelected(
        dbType, 
        'permissionsConfig', 
        ''
    );
    ...
    // 构建规则表达式
    const ruleExpression = PermissionPolicyCenter.buildExpression(
        dbType, 
        selectedPermissions, 
        operator
    );
    ...
    http.get(`/account_classification/api/rules/${id}`)
        .then(data => {
            if (data.success) {
                ...
                // 加载权限配置并设置选中状态
                PermissionPolicyCenter.load(rule.db_type, 'editPermissionsConfig', 'edit')
                    .then(() => {
    ...
    http.get(`/account_classification/api/rules/${id}`)
        .then(data => {
            if (data.success) {
                ...
                // 显示权限配置
                const permissionsHtml = PermissionPolicyCenter.renderDisplay(
                    rule.db_type,
                    rule.rule_expression
                );
...
- 原文件：1880 行
- 抽离权限配置后：约 1200 行
- 新增 permission-policy-center.js：约 600 行
- **净减少：约 80 行**
...
### 步骤 1：创建 permission-policy-center.js
- 实现基础框架
- 实现 MySQL 策略（最常用）
- 测试 MySQL 策略
...
### 步骤 6：清理旧代码
- 删除 `renderPermissionPolicySelector()`（原 `displayPermissionsConfig()`）
...
当前仅 `账户分类管理` 页面依赖权限策略配置，因此无需保留旧接口。完成迁移后应确保以下函数与调用全部移除：

- `renderPermissionPolicySelector`（原 `displayPermissionsConfig`）
- `setSelectedPermissions`
- `displayViewPermissions`
- 任何基于旧 DOM 结构的权限收集逻辑
...
static async load(dbType, containerId, prefix = '') {
    try {
        ...
        // 渲染权限选择器
        const strategy = getPermissionPolicyStrategy(dbType);
